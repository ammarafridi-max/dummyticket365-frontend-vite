...
export default function FlightForm() {
  ...
  const displayPricingOptions = useMemo(() => {
    ...
    return (pricingOptions.length > 0 ? pricingOptions : PRICING_OPTIONS).map((option) => ({
      ...option,
      price: Number((Number(option.price || 0) * rate).toFixed(2)),
    }));
  ...
  useEffect(() => {
    const options = pricingOptions.length > 0 ? pricingOptions : PRICING_OPTIONS;
  ...
  }, [pricingOptions, ticketValidity, updatePricing]);
